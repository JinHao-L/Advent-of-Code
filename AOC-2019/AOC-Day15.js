// Day 15: Oxygen System (Using Intcode)
    ...
    for(let i = 0; i < len; i++) {
        ...
        switch (status) {
            ...
            case "Blocked":
                ...
                plot(loc[0], loc[1], wall_sym);
                continue;
                break;
            case "End":
                plot(loc[0], loc[1], oxy_sym);
                oxygen_X = loc[0];
                oxygen_Y = loc[1];
...
function spread(currX, currY) {
    for(let i = -1; i <= 1; i++) {
        for(let j = -1; j <= 1; j++) {
            ...
            if (val == 1 && map[currY + i][currX + j] == empty_sym) {
                map[currY + i][currX + j] = oxy_sym;
