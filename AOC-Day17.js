...
function Intcode(main, input) {
    ...
    while(i < code.length) {
        ...
        if (opcode == 1) {
            ...
            last_output = code[param1];
            if (code[param1] > 1000) {
                output += code[param1];
            } else {
                output += String.fromCharCode(code[param1]);
            }
            i += 2;
        } else if (opcode == 5) {
            if (code[param1] != 0) {
...
Intcode(code);
let arr = output.split('\n').filter(n => n.length != 0).map(x => x.split(''));

function sum_intersection() {
    let sum = 0;
    for (let height = 1; height < arr.length - 1; height++) {
        for (let width = 1; width < arr[0].length - 1; width++) {
            if (arr[height][width] == '#') {
...
console.log("Part 1: " + sum_intersection() + '\n');
...
const routine = "A, B, A, C, B, A, B, C, C, B \n";
const A = "L, 12, L, 12, R, 4 \n";
const B = "R, 10, R, 6, R, 4, R, 4 \n";
const C = "R, 6, L, 12, L, 12 \n";
// Enable feed to get Part2 Answer
const feed = "y \n";

const input = (routine + A + B + C + feed).split('').filter(x => x != " ").map(x => x.charCodeAt(0));

output = '';
const part2 = Intcode(code, input);
// console.log(output);
